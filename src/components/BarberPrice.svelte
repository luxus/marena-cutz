<script>
  ...
  <button
    ...
    onclick={() => { if (hasMore) openBarber.current = openBarber.current === slug ? '' : slug; }}
  >
    <div class="flex items-start justify-between gap-5">
      <div class="mb-4">
        <h3 class="text-title-md uppercase transition-colors group-hover:!text-primary">{barber.name}</h3>
        {#if barber.position}
  ...
  {#if hasMore}
    <div id={`barber-services-${slug}`}>
      {#if open}
        <div transition:slide={{ duration: 320, easing: (t) => t * t * (3 - 2 * t) }}>
          <!-- Primary service as first row when expanded -->
          {#if primary}
            <div class="price-row grid grid-cols-[1fr_auto] gap-4 border-t border-outline-variant py-3 transition-colors">
              <div>
                <span class="text-body-lg text-on-surface">{primary.name}</span>
                {#if primary.duration}
                  <p class="mt-1 text-label-caps text-on-surface-variant">{primary.duration}</p>
